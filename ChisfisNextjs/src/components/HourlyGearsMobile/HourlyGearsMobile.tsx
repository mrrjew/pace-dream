            ...
            <div className="flex mt-4 gap-4">
                <div className="-mt-1">
                    <div className="flex flex-col -ml-1 gap-4 w-[35px] text-[#666666] text-sm font-semibold">
                        <div className={`flex items-center h-fit ${activeCategory !== "TechGear" && "ml-4"}`}>
                            {renderBlobImage("TechGear")}
                            <p className={`lr -rotate-180 ${activeCategory === "TechGear" && "font-bold"}`} onClick={() => handleCategoryClick("TechGear")}>TechGear</p>
                        </div>
                        <div className={`flex items-center ${activeCategory !== "Music" && "ml-4"}`}>
                            {renderBlobImage("Music")}
                            <p className={`lr -rotate-180  ${activeCategory === "Music" && "font-bold"}`} onClick={() => handleCategoryClick("Music")}>Music</p>
                        </div>
                        <div className={`flex items-center ${activeCategory !== "Photography" && "ml-4"}`}>
                            {renderBlobImage("Photography")}
                            <p className={`lr -rotate-180 ${activeCategory === "Photography" && "font-bold"}`} onClick={() => handleCategoryClick("Photography")}>Photography</p>
                        </div>
                        <div className={`flex items-center ${activeCategory !== "Fashion" && "ml-4"}`}>
                            {renderBlobImage("Fashion")}
                            <p className={`lr -rotate-180 ${activeCategory === "Fashion" && "font-bold"}`} onClick={() => handleCategoryClick("Fashion")}>Fashion</p>
                        </div>
                    </div>
                </div>
                {/* Card */}
