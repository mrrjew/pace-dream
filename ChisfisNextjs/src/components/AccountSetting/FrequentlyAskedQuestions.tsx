...
function FaqItem(props: { title: string; description: string; icon: any }) {
  return (
    <div className="bg-white/45 p-6 rounded-lg  min-h-44 w-full">
      <div className=" rounded-lg mb-2">{props.icon}</div>
      <div>
...
export function FaqSideComponent() {
  const iconclassName =
    "w-12 h-12 text-gray-500 fill-primary-800 text-primary-800 stroke-primary-800 stroke-0";
  ...
  const faqData = [
    ...
    },
    {
      ...
      description:
        "PaceDream only releases contact information for Hosts and guests after a reservation is confirmed.",
