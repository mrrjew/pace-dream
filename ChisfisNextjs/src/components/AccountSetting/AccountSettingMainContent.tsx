...
export type AccountSettingPageType =
  | "Account Settings"
  ...
  | "Privacy & Sharing"
  | "Global Preferences";
const AccountSettingMainContent = ({
  selected,
  onSelect,
}: {
    ...
    <div className="grid grid-cols-1 space-y-16">
      <div className="">
        ...
        {selected !== "Account Settings" && (
          <p className="text-xs text-black opacity-65 font-bold flex items-center gap-1">
            <Link
              href={"#"}
        ...
        <div className="hidden lg:inline-flex flex-col items-start gap-3">
          {userInfo && selected === "Account Settings" && (
            <p className="text-xs">
              {userInfo?.first_name},{" "}
              <span className="text-[#757575]">{userInfo?.email}</span> ·{" "}
      ...
      {selected === "Account Settings" && (
        <AccountSettingSummary selected={selected} onSelect={onSelect} />
      )}
