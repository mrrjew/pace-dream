...
const SectionSubscribe: FC<SectionSubscribe> = ({ className = "" }) => {
  ...
  const handleSubscription = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/newsletter/subscribe`, { email })
  ...
  return (
    ...
    >
        ...
        <div className="flex gap-4 max-md:flex-col justify-center">
          ...
          >
            {loading ? (<div className="flex items-center">
              <svg className="animate-spin h-5 w-5 mr-3 text-violet" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0c4.418 0 8 3.582 8 8s-3.582 8-8 8a1 1 0 01-1-1c0-3.309-2.691-6-6-6s-6 2.691-6 6a1 1 0 01-2 0c0-4.418 3.582-8 8-8v4a1 1 0 01-1 1z"></path>
              </svg>
              Subscribing...
            </div>) : "Subscribe"}
          </button>
      ...
      {message && <p className="text-white font-semibold text-left w-full ml-64">{message}</p>}
    </div>
  );
};
