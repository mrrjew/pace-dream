...
function useLoadGoogleMapScript(){
    return  useJsApiLoader({
        ...
        language: 'en',
        version: 'weekly',
        libraries: ['places','core','geometry','drawing','visualization','places','routes','streetView','elevation'],
    });
}
                        ...
                        {suggestions?.map((suggestion,index) => {
                            return (
                                <div {...getSuggestionItemProps(suggestion, { })}
                                    ...
                                    <span className={`text-xs ${suggestion.active ? 'text-white/85':'text-gray-500'}`}>{suggestion.description}</span>
                                </div>
                            );
                        })}
...
export function GoogleMapLayout(
 ...
 const [address, setAddress] = React.useState<string>(init?.place || "");
     ...
     React.useEffect(()=>{
        ...
        if(navigator.geolocation && isLoaded && !address && !init?.center?.lat && !init?.center?.lng){
  ...
  return (
    ...
    <div className={cn("w-full h-[200px] rounded-lg",className)}>
      {!isMapOnly && <PlacesAutocompleteInput 
       val={address} 
       onSelect={(address,placeId)=>{
        if(isMapOnly) return;
