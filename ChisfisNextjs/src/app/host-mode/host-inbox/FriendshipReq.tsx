  ...
  return (
    <div className="flex flex-col md:flex-row">
      <div className="w-full md:w-1/2 md:border-r-4 md:mobile-res">
        ...
        <div
          className="md:mr-5 flex flex-col column gap-5 my-6 max-h-[400px] overflow-y-auto"
            ...
            incomingRequests?.map((request) => (
              <div key={request.first_name} className="flex items-center justify-center flex-col md:flex-row gap-3 bg-slate-300 py-1 border-collapse rounded-xl">
                ...
                <div className="flex flex-col md:flex-row gap-3">
                  <ButtonPrimary onClick={() => acceptRequest(request._id)}>
                    <p className="text-xs">Accept Request</p>
                  </ButtonPrimary>
                  <ButtonSecondary onClick={() => declineRequest(request._id)}>
                    <p className="text-xs">Decline Request</p>
                  </ButtonSecondary>
                </div>
              </div>
            ))
      ...
      <div className="w-full md:w-1/2">
        ...
        <div
          className="flex flex-col md:ml-5 column gap-5 my-6 max-h-[400px] overflow-y-auto"
          style={{ scrollbarWidth: "none" }}
        >
          {outgoingRequests.length > 0 ? (
            outgoingRequests?.map((request) => (
              <div key={request.first_name} className="flex items-center justify-center flex-col md:flex-row gap-3 bg-slate-300 py-1 border-collapse rounded-xl">
                ...
                <ButtonSecondary onClick={() => cancelRequest(request._id)}>
