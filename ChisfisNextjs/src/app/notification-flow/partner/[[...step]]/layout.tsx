...
const CommonLayout: FC<CommonLayoutProps> = ({ children, params }) => {
  ...
  const nextHref = (
    index < 3
      ? `/notification-flow/partner/${index + 1}`
      : `/notification-flow/partner/${1}`
  ) as Route;
  const backHref = (
    index > 1
      ? `/notification-flow/partner/${index - 1}`
  ...
  const nextBtnText = index > 3 ? "Save All" : "Next";
  ...
  return (
    ...
    >
      <div className="space-y-11">
        ...
        >
          {index > 1 && <ButtonSecondary href={backHref}>back</ButtonSecondary>}
