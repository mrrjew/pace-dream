...
export default function AddListingMainLayoutWrapper({
  ...
  const mutate = useSubmitMutateData<typeof data>({
      ...
      <div className="w-full rounded-lg fixed mt-4 p-2 right-0 left-0 md:right-24 md:left-auto bottom-0 md:bottom-2">
        <div className="flex flex-wrap justify-between md:justify-end  bg-white md:bg-inherit gap-4 items-center p-2 md:p-1 ">
          <div className="flex items-center justify-between gap-2 md:hidden w-full">
            {steps?.map((_, index) => {
              return (
                <div
                  key={index}
                  className={`w-8 h-1 ${currentStep >= index ? "bg-primary-700" : "bg-neutral-300"}`}
                ></div>
              );
            })}
          </div>
          <button
            ...
            className={`${hideBackBtn ? "block md:hidden" : ""} bg-primary-100 text-black py-2 md:py-3 px-8 min-w-fit md:min-w-40 rounded-md disabled:opacity-40 disabled:cursor-not-allowed`}
          >
            Back
          </button>
          <button
            ...
            className="bg-primary-700 text-white py-2 md:py-3 px-4 min-w-fit md:min-w-40 rounded-md"
          >
