...
const PageAddListing1 = ({
  ...
  useEffect(() => {
    const _path = (window?.location?.pathname.split("/").pop() || "Room Stays")
      .replaceAll("-", " ")
      .toLowerCase()
    ...
    if (_path === "hourly rental gear") {
      setPropertyTypes({
        ...
        options: slectionData.hourlyGearType,
      });
      setRoomTypes({
        title: "Select Rental Gear Product",
        options: slectionData.hourlyGearProucts,
      });
      return;
    }
    if (_path === "experience") {
      setPropertyTypes({
        title: "What kind of experience are you offering?",
        options: slectionData.offeringExperience,
      });
    ...
    <>
      <div className="listingSection__wrap ">
        ...
        <div className="space-y-4 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6 bg-white rounded-2xl p-6">
            <FormItem label="What is the name of your property?">
              <Input
                ...
                onChange={(e) =>
                  updateData({ ...data, title: e?.currentTarget?.value })
                }
                name="propertyName"
                type="text"
          ...
          {Number(propertyTypes?.options?.length) +
            Number(roomTypes.options.length) >
            0 && (
            <div className="grid grid-cols-1 gap-4 sm:gap-6 bg-white rounded-2xl p-6">
              {Number(propertyTypes?.options?.length) > 0 && (
                <FormItem label={propertyTypes.title}>
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      updateData({
                        ...
                        details: {
                          ...(data?.details || {}),
                          property_type: e?.currentTarget?.value as any,
                        },
                      });
                    }}
                    name="roomstays"
              ...
              {Number(roomTypes.options.length) > 0 && (
                <FormItem label={roomTypes.title}>
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      updateData({
                        ...
                        details: {
                          ...(data?.details || {}),
                          room_type: e?.currentTarget?.value,
                        },
                      });
                    }}
                    name="roomtype"
