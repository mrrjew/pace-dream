...
import {
  DateImputField,
  TimeSelector,
} from "@/components/ListingComponents/TimeAndDateSelecter";
      ...
      <div className=" ">
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl p-6">
            <p className="flex-1 py-1">How much do you charge per hour?</p>
            <div className="grid gap-4">
              <FormItem label="">
                <Input
                  defaultValue={
                    data?.price?.filter((p) => p.frequency === "hourly")?.[0]
                      ?.amount || ""
                  }
                  onChange={(e) => {
                    // const _others = data?.price?.filter((p)=>p.frequency!=="hourly") || [];
                    const _existing = data?.price?.find(
                      (p) => p.frequency !== "hourly",
                    );
          ...
          <div className="bg-white p-6 rounded-2xl grid grid-cols-1 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <TimeSelector
                initialTime={{
                  min:
                    data?.details?.hourly_rental_time?.start_time?.split(
                      ":",
                    )?.[1] || 0,
                  hour:
                    data?.details?.hourly_rental_time?.start_time?.split(
              ...
              <TimeSelector
                initialTime={{
                  min:
                    data?.details?.hourly_rental_time?.end_time?.split(
                      ":",
                    )?.[1] || 0,
                  hour:
                    data?.details?.hourly_rental_time?.end_time?.split(
