...
const PageAddListing3 = ({
  ...
  const [bedrooms, setBedrooms] = useState<number>(
    (data?.details?.bedroom_count as number) || 1,
  );
  const [bathrooms, setBathrooms] = useState<number>(
    (data?.details?.bathroom_count as number) || 1,
  );
      ...
      <div className=" ">
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl p-6">
            <p className="flex-1 py-1">What is the rental price per month?</p>
            <div className="grid sm:grid-cols-2 gap-4 md:gap-4">
              <FormItem label="">
                <Input
                  defaultValue={
                    data?.price?.filter((p) => p.frequency === "daily")?.[0]
                      ?.amount || ""
                  }
                  onChange={(e) => {
                    const _others =
                      data?.price?.filter((p) => p.frequency !== "daily") || [];
                    const _existing = data?.price?.find(
                      (p) => p.frequency !== "daily",
                    );
                  ...
                  name="daily_rent"
                  type="number"
                  placeholder="$50 Daily rent"
                />
              </FormItem>
              <FormItem label="">
                <Input
                  defaultValue={
                    data?.price?.filter((p) => p.frequency === "weekly")?.[0]
                      ?.amount || ""
                  }
                  onChange={(e) => {
                    const _others =
                      data?.price?.filter((p) => p.frequency !== "weekly") ||
                      [];
                    const _existing = data?.price?.find(
              ...
              <FormItem label="">
                <Input
                  defaultValue={
                    data?.price?.filter((p) => p.frequency === "monthly")?.[0]
                      ?.amount || ""
                  }
                  onChange={(e) => {
                    const _others =
                      data?.price?.filter((p) => p.frequency !== "monthly") ||
                      [];
                    const _existing = data?.price?.find(
              ...
              <FormItem label="">
                <Input
                  defaultValue={
                    data?.price?.filter((p) => p.frequency === "custom")?.[0]
                      ?.amount || ""
                  }
                  onChange={(e) => {
                    const _others =
                      data?.price?.filter((p) => p.frequency !== "custom") ||
                      [];
                    const _existing = data?.price?.find(
                      (p) => p.frequency !== "custom",
                    );
                    const _price = _existing
                      ? {
                          ..._existing,
