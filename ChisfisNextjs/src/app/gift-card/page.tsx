  ...
  const fags = [
    {
      ...
      answer:
        "Gift cards bought on PaceDream.com are digital only. However, physical gift cards are available at participating stores.",
    },
    {
    ...
    <section className="bg-white min-h-screen flex justify-center p-4 flex-col -mb-2">
      <div className="py-24 container">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "url('https://images.unsplash.com/photo-1549116917-bccf55924737?q=80&w=2970&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')",
          }}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-6">
          <div className="col-span-1">
            ...
            <p className="text-xs">
              ...
              <Link className="underline ml-1 font-semibold" href="#">
                help center?
              </Link>
            </p>
          </div>
          ...
          <div className="col-span-1 grid grid-cols-1 gap-6">
            {fags.map((faq, i) => (
              <div
                key={i}
                className={`hover:ring-1 transition-all duration-700 shadow-md w-full md:w-[30em] rounded-md ring-0 p-4 mb-2 ${expanded === i ? "ring-1 ring-primary-400 shadow-primary-400" : ""}`}
              >
                <div
                  onClick={() => handleExpand(i)}
                  className="flex cursor-pointer justify-between py-4 gap-2 items-center"
                >
                  <h3 className="text-xs md:text-sm font-medium flex-1">
                    {faq.question}
                  </h3>
                  <button
                ...
                <p
                  className={`text-xs mr-0 md:mr-4 ${expanded !== i ? "h-0 opacity-0 overflow-hidden" : "h-fit opacity-90"}`}
                >
                  {faq.answer}
                </p>
              </div>
